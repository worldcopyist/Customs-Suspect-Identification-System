import {playStartAnimation} from './intro.js';
...
const titles={dashboard:'工作台',image:'图片识别',camera:'摄像头识别',records:'检测记录',persons:'人员名单',communications:'内部通讯',assistant:'智能助手 / 数字人',users:'用户与权限',providers:'云端服务',agents:'多智能体',knowledge:'业务资料',operations:'运维与审计',bigscreen:'管理数据大屏',chatsearch:'聊天检索与导出',profile:'个人中心'};
...
export async function boot(){
  ...
  try{state.csrf=(await api('/auth/csrf')).csrf_token;state.user=await api('/auth/me');await navigate(location.hash.slice(1)||'dashboard');}catch(err){if(err.code==='PASSWORD_CHANGE_REQUIRED'){state.restricted=true;showAuth('password');}else{await playStartAnimation();showAuth('login',err.code==='UNAUTHENTICATED'?'':err.message);}}
